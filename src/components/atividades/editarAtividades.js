...
function EditarAtividades (props) {
  const { window } = props;
  ...
  const previousValues = JSON.parse(localStorage.getItem("atividade"));
  const[nomeUser, setNomeUser] = useState(previousValues.usuario);
  let {id} = useParams();
  
  const formik = useFormik({
    initialValues:{
      nome: previousValues.nome,
      usuario: previousValues.id_usuario,
      materia: previousValues.id_materia,
      status: previousValues.status,
      cronometro: previousValues.cronometro,
      data_inicio: previousValues.data_inicio,
      data_fim: previousValues.data_fim,
      metodo: previousValues.id_metodo
    },
    onSubmit: values =>{
      ...
      values.id = id;
      console.log(values);
      instance().post("update_atividade.php", values).then((result) =>props.history.goBack());
    },
});
            ...
            <Form onSubmit={formik.handleSubmit}>
                ...
                <Form.Group controlId="cadastro">
                    ...
                    <Form.Control   
                        ...
                        name="cronometro"
                        value={formik.values.cronometro}
                        onChange={formik.handleChange}></Form.Control>
                </Form.Group>
...
export default EditarAtividades;
