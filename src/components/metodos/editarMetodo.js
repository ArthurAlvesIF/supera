...
function EditarMetodo(props) {
  ...
  const formik = useFormik({
      initialValues:{
        nome: JSON.parse(localStorage.getItem("metodo")).nome,
        id: id
      },
      onSubmit: values =>{
        instance().post("update_metodo.php", values).then(()=>props.history.goBack());
      },
  });
    ...
    <ExitToAppIcon style={{position: 'absolute', left: '92%', top: '50%',transform: 'translate(-50%, -50%)', fontSize: 40}} onClick={()=>{localStorage.clear(); history.push("/login"); }}/>
      ...
      <main className={classes.content}>
        ...
        <Card style={{padding:10, flex: 1, marginBottom: 10}}>
            <Card.Title  style={{alignSelf: 'center'}}>Editar o método:</Card.Title>
            <Form onSubmit={formik.handleSubmit}>
                <Form.Group controlId="cadastro">
                    <Form.Label>Nome do métodos</Form.Label>
                    <Form.Control   
                        type="text" 
