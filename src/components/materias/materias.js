...
<ExitToAppIcon style={{position: 'absolute', left: '92%', top: '50%',transform: 'translate(-50%, -50%)', fontSize: 40}} onClick={()=>{localStorage.clear(); history.push("/login"); }}/>
      ...
      <main className={classes.content}>
        ...
        <Card style={{padding:10}}>
            <Card.Title  style={{alignSelf: 'center'}}>Atuais materias cadastradas</Card.Title>
            <ListGroup variant="flush" style={{flex:1}}>
                {materias.map((materia)=>{ return (<ListGroup.Item key={materia.id}><Row><Col sm={10}>{materia.nome}</Col> <Col><Link to={"/materias/editar/" +materia.id} onClick={()=>{localStorage.setItem("materia", JSON.stringify(materia))}}><Button sm={1} variant="warning">Editar</Button></Link></Col><Col><Button sm={1} variant="danger" onClick={()=>{instance().post("delete_materia.php", {id: materia.id}).then(()=>props.history.go(0))}}>Apagar</Button></Col></Row></ListGroup.Item>)})}
            </ListGroup>
