...
function EditarMateria(props) {
  ...
  useEffect(()=>{
    console.log(JSON.parse(localStorage.getItem("materia")).nome);
    instance().get("get_usuario.php").then((result)=>{setUsuarios(result.data); console.log(result.data)});
  },[]);

  const formik = useFormik({
      initialValues:{
        nome: JSON.parse(localStorage.getItem("materia")).nome,
        status: JSON.parse(localStorage.getItem("materia")).status,
        
      },
      validationSchema:schema,
      onSubmit: values =>{
 		let id=0;
        usuarios.map((usuario)=>{if(usuario.email===values.email_usuario)id=usuario.id;});
        values.email_usuario = id;
        instance().post("update_materia.php", values).then((result) => props.history.goBack());
...
<ExitToAppIcon style={{position: 'absolute', left: '92%', top: '50%',transform: 'translate(-50%, -50%)', fontSize: 40}} onClick={()=>{localStorage.clear(); history.push("/login"); }}/>
      ...
      <main className={classes.content}>
        ...
        <Card style={{padding:10, flex: 1, marginBottom: 10}}>
            <Card.Title  style={{alignSelf: 'center'}}>Editar a natéria:</Card.Title>
