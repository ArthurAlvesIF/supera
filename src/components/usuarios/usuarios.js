...
<ExitToAppIcon style={{position: 'absolute', left: '92%', top: '50%',transform: 'translate(-50%, -50%)', fontSize: 40}} onClick={()=>{localStorage.clear(); history.push("/login"); }}/>
      ...
      <main className={classes.content}>
        ...
        <Card style={{padding:10}}>
            <Card.Title  style={{alignSelf: 'center'}}>Atuais usuários cadastrados:</Card.Title>
            <ListGroup variant="flush" style={{flex:1}}>
                {usuarios.map((usuario)=>{ return (<ListGroup.Item key={usuario.id}><Row><Col sm={10}>{usuario.nome}</Col> <Col><Link to={"/usuarios/editar/" +usuario.id} onClick={()=>{localStorage.setItem("usuario", JSON.stringify(usuario))}}><Button sm={1} variant="warning">Editar</Button></Link></Col><Col><Button sm={1} variant="danger" onClick={()=>{instance().post("delete_usuario.php", {id: usuario.id}.then(()=>props.location.reload()))}}>Apagar</Button></Col></Row></ListGroup.Item>)})}
            </ListGroup>
            <div style={{alignSelf: 'center', marginTop: 10}}><Link to ="/usuarios/adicionar"><Button variant="primary" onClick={()=>console.log("clicked")}> Adicionar </Button></Link></div>
        </Card>
      </main>
    </div>
  );
