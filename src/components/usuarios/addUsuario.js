...
function AddUsuario(props) {
  ...
  const formik = useFormik({
      ...
      onSubmit: values =>{
        instance().post("insert_usuario.php", values).then((result) => { try{if(JSON.parse(result.data).erro != null) alert("Email já cadastrado. Tente novamente com outro email."); else this.props.history.goBack();}catch(err){props.history.goBack();console.log(err);console.log(result.data)}});
        console.log(values);
      },
  });
